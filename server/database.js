...
const dbWrapper = {
  async getDB() {
    if (useTurso) {
      ...
      return { type: 'turso', instance: client };
    } else {
      if (!db) {
        const dbPath = process.env.DATABASE_PATH || path.join(__dirname, 'database.db');
        console.log('📁 Using local SQLite database:', dbPath);
        // Dynamic import to prevent Vercel from crashing on native module
        const { default: Database } = await import('better-sqlite3');
        db = new Database(dbPath);
        db.pragma('foreign_keys = ON');
      }
      return { type: 'local', instance: db };
    }
  },

  run: async (sql, params = []) => {
    const { type, instance } = await dbWrapper.getDB();
    if (type === 'turso') {
      return await instance.execute({ sql, args: params });
    }
  ...
  get: async (sql, params = []) => {
    const { type, instance } = await dbWrapper.getDB();
    if (type === 'turso') {
      const result = await instance.execute({ sql, args: params });
  ...
  exec: async (sql) => {
    const { type, instance } = await dbWrapper.getDB();
    if (type === 'turso') {
      return await instance.execute(sql);
    }
...
export async function initDatabase() {
  ...
  try {
    ...
    const servicesCountRes = await dbWrapper.get('SELECT COUNT(*) as count FROM services');
    const servicesCount = servicesCountRes.count || servicesCountRes.COUNT || 0;
    if (servicesCount === 0) {
      const services = [
    ...
    const barbersCountRes = await dbWrapper.get('SELECT COUNT(*) as count FROM barbers');
    const barbersCount = barbersCountRes.count || barbersCountRes.COUNT || 0;
    if (barbersCount === 0) {
      const barbers = [
